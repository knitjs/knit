...
const createBuildTasks = (module) => ({
  ...
  task: ctx => rx.Observable.create(observer => {
    ...
    const pkg = knit.readPkg(module) || {};
    if (!pkg.browser) {
      return observer.complete();
    }
    observer.next('removing old builds');
    del(
      [path.join(needle.paths.modules, module, needle.paths.distStub)]
    ).then(() => {
...
const tasks = [
  {
    ...
    skip: ctx => needle.pkg.ignore && 'project pakcage.json set to ignore: true',
    task: ctx => del(
      [path.join(needle.paths.dist)]
    ).then(() =>
       build.buildProjectUmd('root')
    ),
  },
  {
