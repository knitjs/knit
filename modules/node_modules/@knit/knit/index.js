...
exports.findDependencies = (dir) => {
  const moduleDir = path.join(needle.paths.modules, dir);
  return depchek(moduleDir)
  ...
  try {
    return fs.readdirSync(needle.paths.modules)
      .reduce((acc, d) => {
        ...
        return acc.concat(d)
      }, []).filter(utils.safeFindPkg).filter(pkg => !pkg.private);
  } catch (err) {
...
exports.findModifiedSince = (modules, tag) => {
  ...
  const modified = lines.map(l => {
    ...
    (acc, m) => {
      return acc.includes(m) ? acc : acc.concat(m)
    }
  , []).filter(
...
exports.findAllMissingDependencies = function(modules) {
  return modules.reduce((acc, m) => {
    return Promise.all([acc, exports.findDependencies(m)]).then(
      vals => {
        return vals[0].concat(exports.findMissingDependencies(vals[1], modules))
      }
    ).then(ms => (
      ms.reduce((a, m) => (
...
exports.findAllDependencies = function(modules) {
  return modules.reduce((acc, m) => {
    return Promise.all([acc, exports.findDependencies(m)]).then(
      vals => {
        return vals[0].concat(vals[1], modules)
      }
    ).then(ms => (
      ms.reduce((a, m) => (
        a.includes(m) ? a : a.concat(m)
...
exports.findUpdatedSince = function(modules, tag) {
  ...
  return exports.makeDependencyMap(modules).then(dependencyMap => {
    return exports.resolveCascadingUpdates(modules, dependencyMap, modifiedModules);
  });
};
...
exports.getDependencyVersion = (dep, modules, updated) => {
  if (needle.pkg.dependencies && needle.pkg.dependencies[dep]) {
    ...
    const pkg = utils.safeFindPkg(path.join(needle.paths.modules, dep));
    if (pkg && pkg.version) {
      return pkg.version;
    }
  }
    ...
    Object.assign({},
      {
        ...
        dependencies: used.concat(pkg.dependencies || []).filter(d => (
...
exports.updateModulePkgBrowser = (pkg) => (
  Object.assign({},
    pkg,
    {
      ...
      files: [...pkg.files, needle.paths.dist],
    }
  )
)
