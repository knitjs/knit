...
export type TPublishedVersion = (module: string) => Promise<string>;
const npmPublishedVersion: TPublishedVersion = module => execa.stdout('npm', ['info', module, '--json'])
  .then(data => (
      JSON.parse(data)['dist-tags'].latest
  ));
// const yarnPublishedVersion = (module: string) => execa.stdout('yarn', ['info', module, '--json']).then(data => (
//     JSON.parse(`[${data.replace(/}\s{/, '},{')}]`)[0].data.version
// ));
export const publishedVersion: TPublishedVersion = npmPublishedVersion; // yarn version isn't as good
