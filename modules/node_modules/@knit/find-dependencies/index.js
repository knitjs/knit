...
type TFindAllMissingDependencies = (d: string, m: TModules, rootPkg: TPkgJson) => Promise<TModules>;
export const findAllMissingDependencies: TFindAllMissingDependencies = (dir, modules, rootPkg) => (
  findAllDependencies(dir, modules).then(using => (
    findMissingDependencies(using, modules, rootPkg.dependencies)
  ))
);

type TFindAllUnusedDependencies = (d: string, m: TModules, rootPkg: TPkgJson) => Promise<TModules>;
export const findAllUnusedDependencies: TFindAllUnusedDependencies = (dir, modules, rootPkg) => (
  findAllDependencies(dir, modules).then(using => (
    findUnusedDependencies(using, modules, rootPkg.dependencies)
  ))
);

type TMakeDependencyMap = (d: string, m: TModules) => Promise<{[k: string]: TModules}>;
