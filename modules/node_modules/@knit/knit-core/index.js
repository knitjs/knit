...
type TReadPkg = (d: string, p: TPaths) => TPkgJson;
export const readPkg: TReadPkg = (dir, paths) => {
  try {
    return rp.sync(path.join(paths.modules, dir), {
      normalize: false,
    });
  } catch (err) {
    throw {
      message: `could not find a \`package.json\` in ${path.join(paths.modulesStub, dir)}.`,
...
type TFindDependencies = (dir: string, paths: TPaths) => Promise<TModules>;
export const findDependencies: TFindDependencies = (dir, paths) => (
   depcheck(path.join(paths.modules, dir))
    .then(res => Object.keys(res.using))
    .then(using => {
      const pkg = readPkg(dir, paths);
...
export const findModules: TFindModules = (paths) => {
  try {
    return fs.readdirSync(paths.modules)
      .reduce((acc, d) => (
        isScoped(d)
          ? acc.concat(fs.readdirSync(path.join(paths.modules, d)).map(dir => path.join(d, dir)))
          : acc.concat(d)
      ), []).filter(d => fs.statSync(path.join(paths.modules, d)).isDirectory());
  } catch (err) {
    throw {
...
export const findAllDependencies: TFindAllDependencies = (modules, paths) => (
   modules.reduce((acc, mod) =>
     Promise.all([acc, findDependencies(mod, paths)]).then(
      vals => (
...
export const makeDependencyMap: TMakeDependencyMap = (modules, paths) => (
  modules.reduce((acc, m) => (
    Promise.all([acc, findDependencies(m, paths)]).then(
      vals => ({
        ...vals[0],
        [m]: vals[1],
      })
...
export const updateModulePkg: TUpdateModulePkg = (modules, params) => {
  ...
  return ({
    ...
    ...params.pkg,
    ...{
      main: path.join(params.paths.lib, 'index.js'),
...
export const updateModulePkgBrowser: TUpdateModulePkgBrowser = (pkg, paths) => ({
  ...pkg,
  ...{
    'jsnext:main': path.join(paths.jsnext, 'index.js'),
    browser: path.join(paths.umd, 'index.min.js'),
  },
});
