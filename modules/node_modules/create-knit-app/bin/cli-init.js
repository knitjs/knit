...
module.exports = argv => {
  ...
  pify(fs.stat)(cwd).then(() => {
    ...
    return new Listr([
      ...
      .then(() => execa('yarn', ['init', '--yes'], { cwd }))
      .then(() => pify(fs.copy)(path.resolve(path.join(__dirname, '..', 'template', 'package.json')), cwd))
      .then(() => execa('git', ['init'], { cwd }))
      .then(() => {
        log.info('installing dependencies...');
        return execa(
          'yarn', ['add', 'react', 'glamor'], { cwd, stdio: 'inherit' }
        ).then(() =>
      ...
      .then(() => {
        ...
        return execa(
          'yarn', [
            ...
            '@knit/babel-preset-socks',
            '@knit/jest-config-socks',
            '@knit/mittens-develop',
            '@knit/mittens-publish',
          ], { cwd, stdio: 'inherit' }
        );
      })
      .then(() => {
