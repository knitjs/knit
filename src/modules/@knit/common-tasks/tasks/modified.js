...
import { findModifiedSince, findModifiedPackages } from '@knit/find-modified-packages';
...
const tasks = [
  ...
  },
  {
    ...
    skip: (ctx: TCtx) => ctx.modified && `${ctx.modified.length} modules already found.`,
    task: (ctx: TCtx) => {
      if (ctx.tag) {
        const modifiedSince = findModifiedSince(ctx.workingDir || needle.paths.modulesStub, ctx.public, ctx.tag);
        return findModifiedPackages(ctx.workingDir || needle.paths.modulesStub, ctx.public, modifiedSince).then(modified => {
